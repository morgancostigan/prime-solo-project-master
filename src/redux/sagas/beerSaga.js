...
function* deleteBeer(action) {
    console.log('in deleteBeer, action', action);
    try {
        const config = {
        ...
        const response = yield axios.delete(`api/beer/${action.payload.beer_id}`, config);

        // now that the session has given us a user object
        // with an id and username set the client-side user object to let
        // the client-side code know the user is logged in
        yield put({ type: 'FETCH_OUR_BEER', refresh: action.refresh });
    } catch (error) {
        console.log('Beer delete request failed', error);
    }
}//end deleteBeer
...
function* fetchOurBeer(action) {
    try {
        ...
        // If a user is logged in, this will return their information
        // from the server session (req.user)
        const response = yield axios.get(`api/beer/portfolio?id=${action.refresh}`, config);
